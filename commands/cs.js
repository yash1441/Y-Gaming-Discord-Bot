...
module.exports = {
			...
			subcommand
				...
				.addStringOption((option) =>
					option
						.setName("steam-id")
						.setDescription(
							"Enter the Steam ID of the player. Example: STEAM_1:1:123456789",
						)
						.setRequired(true),
				),
		)
	...
	async autocomplete(interaction) {
		...
		if (focusedOption.name === "item-name") {
			...
			await interaction
				.respond(options.map((choice) => ({ name: choice, value: choice })))
				.then(() => {
					wears = null;
				});
			...
			if (!itemName || !choices.includes(itemName)) {
				...
				const filtered = wears.filter((wears) => {
					...
					return (
						words.every((word) => wearsLower.includes(word)) ||
						words.some((word) => wearsLower.includes(word))
					);
				});
		...
		if (subCommand === "rank") {
			...
			if (!steamId.startsWith("STEAM_")) {
				return await interaction.editReply({
					content: `\`${steamId}\` is an invalid Steam ID.`,
				});
			}
			...
			if (embeds == 0)
				return await interaction.editReply({
					content: `No matches have been added for \`${steamId}\`.`,
				});
			if (embeds == -1)
				return await interaction.editReply({
					content: `An error occured while fetching data for \`${steamId}\`.`,
				});
			...
			if (!choices.includes(itemName) && !wears.includes(itemWear))
				return await interaction.editReply({
					content: "Invalid item! Please try again.",
				});
			let skin = wears.includes("NA")
				? itemName
				: itemName + " (" + itemWear + ")";
			...
			if (!skinData || skinData.error)
				return await interaction.editReply({ content: skinData.error });
			...
			const embed = new EmbedBuilder()
				...
				.addFields(
					{
						name: "Buff Price (BUY)",
						value: "₹" + (parseInt(skinData.buyPrice) * 11.5).toString(),
						inline: true,
					},
					{
						name: "Buff Price (SELL)",
						value: "₹" + (parseInt(skinData.sellPrice) * 11.5).toString(),
						inline: true,
					},
					{
						name: "Steam Price",
						value: "₹" + (parseInt(skinData.steamPrice) * 11.5).toString(),
						inline: false,
					},
				)
